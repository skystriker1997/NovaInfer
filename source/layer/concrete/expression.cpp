...
namespace nova_infer {
    ...
    LayerExpression::LayerExpression(std::string_view name,
    ...
    void LayerExpression::Parse() {
        ...
        while(lptr != expression_.end()) {
            ...
            if(c == '@') {
                ...
                << "failed to parse expression; unidentified operator starting with: " + std::string{lptr, rptr++};
                ...
                check_(false)
                << "failed to parse expression; unidentified item starting with: " + std::string{lptr, lptr + 1};
            }
        }
    }
    ...
    std::shared_ptr<LayerExpression> MakeLayerExpression(pnnx::Operator *opt) {
        Check check;
        check(!opt->inputs.empty()) << "failed to create layer expression; need at least one tensor as input";
        check(opt->outputs.size()==1) << "failed to create layer expression; should produce only one tensor as output";
        ...
        return std::make_shared<LayerExpression>(opt->name, std::move(input_names), std::move(output_name), expr->second.s);
    };
