...
namespace nova_infer {
    ...
    LayerReLU::LayerReLU(std::string_view name, std::vector<std::string> input_name, std::vector<std::string> output_name)
    {
    ...
    std::shared_ptr<LayerReLU> MakeLayerReLU(pnnx::Operator *opt) {
        ...
        check(opt->inputs.size()==1) << "failed to create layer relu; only accept one tensor as input";
        ...
        return std::make_shared<LayerReLU>(opt->name, std::move(input_name), std::move(output_name));
