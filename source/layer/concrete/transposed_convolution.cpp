...
namespace nova_infer {
    LayerTransposedConvolution::LayerTransposedConvolution(std::string_view name,
    ...
    std::shared_ptr<LayerTransposedConvolution> MakeLayerTransposedConvolution(pnnx::Operator *opt) {
        ...
        check(opt->inputs.size()==1) << "failed to create layer transposed convolution; only accept one tensor as input";
        ...
        check(dilation->second.ai.size()==2) << "failed to create layer transposed convolution; parameter dilation should have two coefficients";
        ...
        check(padding->second.ai.size()==2) << "failed to create layer transposed convolution; parameter padding should have two coefficients";
        ...
        check(stride->second.ai.size()==2) << "failed to create layer transposed convolution; parameter stride should have two coefficients";
        ...
        check(kernel_size->second.ai.size()==2) << "failed to create layer transposed convolution; parameter kernel_size should have two coefficients";
        ...
        check(output_padding->second.ai.size()==2) << "failed to create layer transposed convolution; parameter output_padding should have two coefficients";
        ...
        check(kernels*kernel_c*non_empty_h*non_empty_w == weights_converted.size()) << "failed to create layer transposed convolution; kernel shape is mismatching with number of coefficients";
        ...
        return std::make_shared<LayerTransposedConvolution>(opt->name,
                                                            std::move(input_name),
