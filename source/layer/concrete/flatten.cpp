...
namespace nova_infer {
    LayerFlatten::LayerFlatten(std::string_view name,
    ...
    void LayerFlatten::Forward() {
        ...
        check_(start < end) << "failed to execute flatten layer; the start dimension must be before end dimension";
        check_(start >= 1 && end <= 3)
        << "failed to execute flatten layer; the start dimension cannot be before dimension 1 and the end dimension cannot be after dimension 3";
    ...
    std::shared_ptr<LayerFlatten> MakeLayerFlatten(pnnx::Operator *opt) {
        Check check;
        check(opt->inputs.size()==1) << "failed to create layer flatten; only accept one tensor as input";
        ...
        return std::make_shared<LayerFlatten>(opt->name, std::move(input_name), std::move(output_name), start_dim->second.i, end_dim->second.i);
    };
